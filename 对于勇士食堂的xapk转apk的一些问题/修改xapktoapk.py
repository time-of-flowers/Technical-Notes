...
def merge_apk_assets(dir_apk_main, dir_apk_with_asset_pack):
    ...
    for root, dirs, files in os.walk(asset_pack_dir):
        ...
        for file in files:
            ...
            asset_pack_file_path_rel = asset_pack_file_path_abs[len(asset_pack_dir):].lstrip(os.sep)
...
def unpack_apk(path_dir_tmp, apk_file, number_current, number_total):
    ...
    if check_if_executable_exists_in_path('apktool'):
        rc = execute_command_subprocess(['apktool.bat', 'd', '-s', apk_file])
        if rc != 0:
            raise Exception("failed to unpack %s" % apk_file)
    else:
        params = [get_path_to_batch('apktool.bat'), 'd', '-s', apk_file]
...
def pack_apk(path_dir_tmp, main_apk_dir):
    ...
    os.chdir(path_dir_tmp)
    if check_if_executable_exists_in_path('apktool'):
        rc = execute_command_subprocess(['apktool.bat', 'b', main_apk_dir])
        if rc != 0:
            raise Exception("failed to pack apk")
    else:
        params = [get_path_to_batch('apktool.bat'), 'b', main_apk_dir]
        p = Popen(list2cmdline(params))
        _, stderr = p.communicate()
        if stderr is not None:
...
def load_sign_properties():
    ...
    path_sign_config_file = os.path.normpath(path_sign_config_file)
...
def main():
    ...
    with open(path_xapk_manifest, 'r', encoding='utf-8') as file:
